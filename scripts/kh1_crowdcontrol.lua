...
-- Spawns a creature the game ALREADY has loaded in this room, chosen at random.
--
-- Every other spawn effect names a fixed creature, so it fails whenever that creature is not loaded
-- AND there is no free run of consecutive slots to load it into -- which is most of the time in a
-- busy room, since a room's own roster routinely claims 40-50 of the 50 usable slots. Reusing
-- something already resident needs no free slots at all, so this succeeds where those cannot.
--
-- Slot-table layout: `loadedSpeciesPtrTable` points at slot 0's POINTER field, which sits 0x48 into
-- a 0x50-byte entry, so the entry itself begins 0x48 earlier.
local SLOT_TABLE_BASE = loadedSpeciesPtrTable - 0x48
local SLOT_STRIDE, SLOT_COUNT = 0x50, 64
local SLOT_STATE_OFF, SLOT_NAME_OFF, SLOT_NAME_LEN = 0x3, 0x4, 0x20
local SLOT_STATE_READY = 6
local BLOB_STRIDE, BLOB_HEADER_FIRST_SECTION = 0x40000, 128

local function loaded_creature_models()
    local out = {}
    for s = 0, SLOT_COUNT - 1 do
        local entry = SLOT_TABLE_BASE + s * SLOT_STRIDE
        -- state==6 plus a cached name is necessary but NOT sufficient: neither field is ever cleared
        -- when a room stops using a slot, so a run MEMBER still carries the name of whatever was
        -- last a run start there. Only a genuine run start has its blob header at its own base, so
        -- that is the one field that cannot be stale.
        if ReadByte(entry + SLOT_STATE_OFF) == SLOT_STATE_READY
           and ReadInt(speciesResourceTable + s * BLOB_STRIDE + 4) == BLOB_HEADER_FIRST_SECTION then
            local bytes = ReadArray(entry + SLOT_NAME_OFF, SLOT_NAME_LEN)
            local chars = {}
            for i = 1, #bytes do
            ...
            -- kh1_creature_data covers only spawnable creatures, so this also filters out Sora and
            -- the party, who hold the low slots and are emphatically not wanted here.
            if kh1_creature_data[name] then out[#out + 1] = name end
        end
    end
    return out
end

effect_handlers["spawn_random_loaded"] = {
    apply = function(request_id)
        local models = loaded_creature_models()
        if #models == 0 then
            return false, "spawn_enemy: nothing spawnable is loaded in this room right now"
        end
        local model_path = models[math.random(#models)]
        local motion_path = kh1_creature_data[model_path].motionPath
        local pos = kh1.get_sora_pos()
        local angle = math.random() * 2 * math.pi
        local radius = 250 + math.random() * 150
        local x = pos["X"] + math.cos(angle) * radius
        ...
        kh1.spawn_enemy(model_path, motion_path, x, pos["Y"], z, function(ok, result)
            ...
            if ok and type(result) == "number" then
                detail = string.format(
                    "spawn_enemy(\"%s\") = %s / entity=0x%X (random pick of %d loaded)",
                    model_path, tostring(ok), math.floor(result), #models)
            else
                detail = string.format("spawn_enemy(\"%s\") = %s / %s (random pick of %d loaded)",
                    model_path, tostring(ok), tostring(result), #models)
            end
            log(detail)
            send_response(request_id, ok, detail)
        end)
