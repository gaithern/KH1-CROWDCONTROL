...
| `message` | Shows a **preset** message (viewer picks one of 16 short options — GG, Nice!, Skill issue, etc., see `MessagePreset` in `pack/KH1CrowdControlPack.cs`) via `kh1.show_custom_item_popup` — the map-prize pickup popup, repurposed to show custom text instead of an item name | Untested end-to-end — switched from `open_text_box` (wasn't working), then from free text to a preset list after Crowd Control's team confirmed on Discord this connector has **no free-text input at all**, only a numeric `Quantity` slider and `Parameters` (pick-one-from-a-list, or hex color) |
