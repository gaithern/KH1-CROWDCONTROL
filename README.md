...
See the `effect_handlers` table in `scripts/kh1_crowdcontrol.lua` (111 effects total) and the matching entries in `pack/KH1CrowdControlPack.cs`.
...
| `sound_2` .. `sound_76` (74 effects, all of 2-76 except 31) | Plays KH1 sound effect id N via `kh1.play_se2(N, 0)` | **UNTESTED, may crash** — a deliberate accepted tradeoff for redemption variety, not an oversight; see the note below the table |
...
| `message` | Viewer picks one of 16 presets (GG, Nice!, Skill issue, etc. — see `MESSAGE_PRESETS`); shows it via `kh1.show_custom_item_popup` — the map-prize pickup popup, repurposed to show custom text instead of an item name | **Confirmed live** — fired successfully in the SDK's Test Effects tool. History: switched from `open_text_box` (wasn't working) to a `Parameters` picker (Crowd Control's team confirmed on Discord there's **no free-text input at all** on this connector, only `Quantity` and `Parameters`/pick-from-list) — briefly mis-implemented as 16 separately-*coded* effects after misreading the SDK's Output-panel display text, until a real logged request confirmed the wire format is `{"code":"message","parameters":{"text":{"value":"gg",...}}}`. A "works once, then nothing for ~60s" observation turned out to be the SDK's own default per-effect cooldown when `SessionCooldown` is left unset (a real `Effect` property — confirmed via `WarpWorld/CCPack-PC-DeepRockGalactic`'s real usage); now explicitly set to `SessionCooldown = 3`. |
...
**On `sound_2`..`sound_76`:** this repo's original rule was "only ship `se_id` values you've personally confirmed live" — `play_se2`'s own comment in `kh1_lua_library.lua` documents that `se_id=1` (inside the otherwise-plausible 1-76 range) crashed the game outright through this exact call path, confirmed live 2026-07-12. The range effects above are a **deliberate, explicit exception** to that rule (se_id=1 itself is excluded; every other id 2-76 is untested and may crash) — a conscious tradeoff for redemption variety made after weighing the risk, not an oversight. If a stream-ending crash from an unverified sound id isn't an acceptable risk for your setup, remove the `SOUND_RANGE_MIN`/`SOUND_RANGE_MAX` loop in `kh1_crowdcontrol.lua` (and the matching entries in the pack files) and keep only `sound_31`.
