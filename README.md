...
| `message_gg`, `message_nice`, `message_oops`, `message_uhoh`, `message_nooo`, `message_yay`, `message_hello`, `message_whoops`, `message_sotrue`, `message_skillissue`, `message_chaos`, `message_goodluck`, `message_badluck`, `message_tryagain`, `message_wtake`, `message_ltake` (16 effects) | Shows a fixed preset message via `kh1.show_custom_item_popup` — the map-prize pickup popup, repurposed to show custom text instead of an item name | Untested end-to-end. History: switched from `open_text_box` (wasn't working) to free text via a `Parameters` field (Crowd Control's team confirmed on Discord there's **no free-text input at all** on this connector, only `Quantity` and `Parameters`/pick-from-list), then live-tested (2026-07-13) and found `Parameters`-based effects don't arrive as `code="message"` + a parameters field at all — Crowd Control sends a **compound code** per option (`message_GG`, not `message`) — so this is now 16 separately-coded effects, matching how `MessagePreset` in `pack/KH1CrowdControlPack.cs` auto-generates them |
